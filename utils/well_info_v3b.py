...
cc = np.column_stack((np.arange(len(dbs_c_)), dbs_c_, np.ones(len(dbs_c_))*dbs.max()))


#-----
# Writes a file for later use
well.to_csv('wells_2_pf_v3b.csv', index=True)

np.savetxt('plm_grid_info_v3b.csv', cc, fmt='%.4f', delimiter=',', header='Z_layer_num,Depth_bls,Total_Depth',comments='')
